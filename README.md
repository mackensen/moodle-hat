...
These instructions assume a basic familiarity with Vagrant. Run `vagrant up` after the initial clone and `vagrant provision` after updates. The Moodle instance may be accessed at [http://local31.moodle.dev](http://local31.moodle.dev).
...
Profiles are logged to `/tmp` on the virtual machine and may be inspected at [http://webgrind31.moodle.dev](http://webgrind31.moodle.dev), which runs [Webgrind](https://github.com/jokkedk/webgrind).

#### Cron

Cron is enabled by default under the `www-data` user. It logs to `/var/www/cron`. You may view the logs at [http://cron31.moodle.dev](http://cron31.moodle.dev).
